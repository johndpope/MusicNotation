...
+ (void)load {
  // Each category's +load is called on startup. This method installs our
  // getter/setter if NSUserDefaults doesn't have one.
  ...
  if (![self instancesRespondToSelector:getter]) {
    IMP imp = imp_implementationWithBlock(
        (CAST_TO_BLOCK) ^
        NSObject * (NSUserDefaults * self, id subscript) {
  ...
  if (![self instancesRespondToSelector:setter]) {
    IMP imp = imp_implementationWithBlock(
        (CAST_TO_BLOCK) ^
        void(NSUserDefaults * self, id object, id<NSCopying> subscript) {
