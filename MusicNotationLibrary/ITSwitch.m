...
- (void)drawFocusRingMask
{
    ...
    NSBezierPath* path =
        [NSBezierPath bezierPathWithRoundedRect:[self bounds] xRadius:cornerRadius yRadius:cornerRadius];
...
- (void)reloadLayer
{
    ...
    [CATransaction setAnimationDuration:kAnimationDuration];
    {
        ...
        //        _backgroundLayer.borderWidth = (YES || self.isActive || self.isOn) ? NSHeight(_backgroundLayer.bounds)
        //        / 2 : kBorderLineWidth;
...
- (CGRect)rectForKnob
{
    ...
    CGFloat width = ![self isActive]
                        ? (NSWidth(_backgroundLayer.bounds) - 2.f * kBorderLineWidth) * 1.f / kGoldenRatio
    ...
    return (CGRect){
        .size.width = width, .size.height = height, .origin.x = x, .origin.y = kBorderLineWidth,
    };
}
